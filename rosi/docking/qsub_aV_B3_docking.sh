...
#$ -o /netapp/home/mmravic/CHAMP/logs                      #-- output directory (fill in)
...
for pdb in /home/xray/CHAMP/modeling_aVb3/rosi/docking/prepack*.pdb
	...
	if [ $inx -eq $taskID ]
			then
				...
				~/rosetta/source/bin/mp_dock.linuxgccrelease -in:file:s $pdb -score:weights mpframework_docking_fa_2015.wts -mp:setup:spanfiles aV_B3_apart.span -mp:scoring:hbond -packing:pack_missing_sidechains 0 -docking:partners A_B -nstruct 1 -out:overwrite -out:prefix outputs/ -ignore_zero_occupancy false
				break 
